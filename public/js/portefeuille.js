...
var portefeuille = new Vue({
  ...
  created: function() {
    var vm = this;
    this.$http.get('api/monnaies').then(function(res) {
  ...
  methods: {
    updateGraph:function(){
      var vm = this;
      if(vm.selected != "null"){
        this.$http.get('api/portefeuille/'+ vm.selected).then(function(res) {
    ...
    buy:function(){
      ...
      if(vm.selected != "null") {
        if(this.$refs.input_buy.value.length != 0){
          this.$http.post('api/portefeuille/'+ vm.selected, {amount: this.$refs.input_buy.value}).then(function(res) {
    ...
    sell:function(){
      ...
      if(vm.selected != "null") {
        if(this.$refs.input_sell.value.length != 0){
          this.$http.post('api/portefeuille/'+ vm.selected+'/vendre', {amount: this.$refs.input_sell.value}).then(function(res) {
            if(res.data.success == true){
    ...
    updatePortefeuille:function(){
      ...
      var vm = this;
      this.$http.get('api/portefeuille/').then(function(res) {
